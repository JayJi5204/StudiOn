...
public class SignalingHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        switch (signalingMessage.getType()) {
            ...
            case "status-change" -> handleStatusChange(session, roomId, userId, signalingMessage); // 추가됨
    ...
    private void handleJoin(WebSocketSession session, String roomId, String userId, String nickName) throws Exception {
        ...
        joinMsg.setNickName(nickName);
        broadcast(roomId, userId, joinMsg);
    }

    private void handleStatusChange(WebSocketSession session, String roomId, String userId, SignalingMessage message) throws Exception {
        log.info("status-change roomId={}, userId={}", roomId, userId);
    ...
    public void forceCloseRoom(String roomId) throws Exception {
        Map<String, WebSocketSession> room = rooms.get(roomId);
        if (room == null) return;

        SignalingMessage forceCloseMsg = new SignalingMessage();
        forceCloseMsg.setType("force-close");
        forceCloseMsg.setRoomId(roomId);
        String json = objectMapper.writeValueAsString(forceCloseMsg);

        for (Map.Entry<String, WebSocketSession> entry : room.entrySet()) {
            if (entry.getValue().isOpen()) {
                entry.getValue().sendMessage(new TextMessage(json));
                entry.getValue().close(CloseStatus.NORMAL);
            }
        }

        rooms.remove(roomId);
        log.info("방 강제 종료 roomId={}", roomId);
    }
}