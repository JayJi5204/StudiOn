...
public class CommentServiceImpl implements CommentService {
    ...
    public CreateResponse create(CreateRequest dto, HttpServletRequest request) {
        ...
        // 댓글 수 증가 이벤트
        kafkaProducer.send("comment.created", new CommentCreatedEvent(
    ...
    public List<GetResponse> getCommentWithBoardId(Long boardId, HttpServletRequest request) {
        Long userId = null;
        try {
            userId = SecurityUtil.getCurrentUserId(request);
        } catch (Exception e) {
            // Feign 호출이거나 비로그인
        }
        final Long finalUserId = userId;
        return commentRepository.findAllByBoardId(boardId).stream()
                .map(entity -> GetResponse.from(
                        entity,
                        commentCountService.getLikeCount(entity.getCommentId()),
                        finalUserId != null && commentCountService.isLiked(entity.getCommentId(), finalUserId)
                ))
                .toList();
    }
