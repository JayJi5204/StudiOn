...
import { COMMENT_DB } from "./DB/commentDB";
...
const testViewCount = http.patch(`${API_URL_COMMUNITY_BOARD}/post/:id/views`, async ({ params }) => {
  ...
  if (postIndex !== -1) {
    ...
    POSTS_DB.posts[postIndex].viewCount += 1;
    
    // 로그로 확인해보기 (터미널이나 브라우저 콘솔)
    console.log(`Post ${id} 조회수 증가 완료:`, POSTS_DB.posts[postIndex].viewCount);
...
const testDeleteComment = http.delete(
  ...
  ({ params }) => {
    ...
    const commentIdx = COMMENT_DB.comments.findIndex(comment => comment.commentId === Number(commentId));
    ...
    if (commentIdx !== -1) {
      ...
      // );
      COMMENT_DB.comments.filter(comment => comment.commentId !== Number(commentId))
...
const testUpdateComment = http.put(`${API_URL_COMMUNITY_BOARD}/posts/:id/comments`,async ({request})=>{

  const editComment = await request.json() as IComment;
  const idx = COMMENT_DB.comments.findIndex(comment => comment.commentId === editComment.commentId)
    if (idx !== -1){
      COMMENT_DB.comments[idx].content = editComment.content;
      return HttpResponse.json(COMMENT_DB.comments[idx], {status:200})
    }
    return new HttpResponse(null, { status: 404 });
  }
);
...
const testCreateComment = http.post(
  ...
  async ({ params, request }) => {
    ...
    COMMENT_DB.comments.push(newComment);
    ...
    return HttpResponse.json(
      {
        ...
        bio: user.bio,
        role: user.role,
        profieAvatar: user.profileAvatar,
