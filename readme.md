...
- 브라우저 종료 시 자동 퇴장 처리
- 공부시간 자동 측정 및 기록 (초 단위)
- 관리자 강제 종료 시 WebSocket으로 즉시 연결 해제

### 스터디 그룹

- 스터디 그룹 생성 / 수정 / 삭제
- 카테고리 필터링 (취업 / 자격증 / 어학 / 기타)
- 키워드 검색 (그룹명)
- 초대코드 기반 참여
- 가입 신청 / 수락 / 거절 / 추방 / 탈퇴
- 회장 위임
...
- 1대1 실시간 채팅 (WebSocket + STOMP + Redis Pub/Sub)
- 그룹 실시간 채팅 (WebSocket + STOMP + Redis Pub/Sub)
- 닉네임으로 유저 검색 후 채팅 시작

### 게시판

- 게시글 CRUD / 카테고리 / 키워드 검색 / 페이지네이션
- 조회수 / 좋아요 / 댓글 수 Redis 캐싱
- 무한 댓글 (Path 기반 계층 구조)
- 조회수 / 좋아요 기반 인기 게시글 랭킹

### 랭킹

- 공부시간 기반 유저 랭킹 (Redis Sorted Set)
...
- SSE 기반 실시간 알림
- 댓글 / 채팅 / 스터디룸 초대 / 강제 종료 / 스터디 그룹 / 게시글 강제 삭제 알림
...
- 유저 검색 / 강제 탈퇴 (Redis refreshToken 삭제 포함)
- 게시글 강제 삭제 (작성자 알람 발행)
- 스터디룸 강제 폐쇄 (WebSocket 즉시 강제 종료 + 참여자 알람)
...
│  alarm-service   │  studyGroup-service                          │
└─────────────────────────────────────────────────────────────────┘
        ↓                  ↓                    ↓
     [MySQL]            [Redis]              [Kafka]
...
│   │   ├── room/             # 스터디룸 / WebRTC
│   │   ├── alarm/            # 실시간 알림
│   │   └── studyGroup/       # 스터디 그룹 (신규)
│   └── common/               # 공통 모듈
└── Frontend/
    └── src/
        ├── api/              # API 호출 모듈
        ├── components/       # 재사용 컴포넌트
        ├── hooks/            # 커스텀 훅 (WebRTC, STOMP)
        ├── pages/            # 페이지 컴포넌트
        ├── router/           # 라우터 설정
...
- Docker Desktop 설치 (메모리 8GB 이상 권장)
- Docker Compose 설치
